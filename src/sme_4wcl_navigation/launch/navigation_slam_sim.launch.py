...
def generate_launch_description():
    ...
    navigation_slam_launch_file = os.path.join(pkg_sme_navigation, 'launch', 'navigation_slam.launch.py')
    ...
    # SLAM + Navigation nodes (RSP, EKF, slam_toolbox, Nav2 without AMCL)
    navigation_slam = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(navigation_slam_launch_file),
        launch_arguments={
            'use_sim_time': use_sim_time,
    ...
    # We delay Navigation+SLAM by 2s so it doesn't crash from clock issues
    ld.add_action(TimerAction(             
        period=2.0,
        actions=[navigation_slam]
    ))
    
    ld.add_action(spawn)                   # 3. Spawn robot after 3s
    
    ld.add_action(TimerAction(             # 4. Start RViz after 5s
        period=5.0,
        actions=[rviz]
    ))
